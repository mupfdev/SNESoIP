...
int main() {
	...
	// Test sequence.
	snesIO seq[6] = { 0xffef, 0xffbf, 0xffdf, 0x6e0f, 0x2f0e, 0x4e0f };
	int seqi = 0;
	...
	while (1) {
		...
		sendto(sockfd, &seq[seqi], 2, 0, (struct sockaddr *)&clientAddr, sizeof(clientAddr));
		printf("index=%d value=%04x\n", seqi, seq[seqi]);
		seqi++;
		if (seqi > 5) seqi = 0;
		...
		if (clientCache != clientData[clientID]) {
			...
			//printf("%s Client%i: 0x%x\n", timestamp, clientID, clientData[clientID]);
		}
	}
